...
export function parseClaudeUsageLine(line: string): TranscriptUsageRow | undefined {
  ...
  if (!message.usage || typeof message.usage !== "object" || typeof row.timestamp !== "string") return undefined;
  ...
  return {
    agent_id: sessionId ? `claude-${createHash("sha256").update(sessionId).digest("hex").slice(0, 24)}` : `claude:${messageId ?? requestId ?? row.timestamp}`,
    project: cwd ? basename(cwd) : "claude",
    runtime: "claude",
    model,
    tokens, uncached_input_tokens: uncachedInputTokens, cached_input_tokens: cachedInputTokens,
    cache_creation_tokens: cacheCreationTokens, output_tokens: outputTokens, reasoning_tokens: 0,
...
async function listJsonl(root: string, sinceMs: number, found: string[] = []): Promise<string[]> {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) await listJsonl(path, sinceMs, found);
      ...
      try { if ((await stat(path)).mtimeMs >= sinceMs) found.push(path); } catch { /* Rotated during scan. */ }
    }
  }
  return found;
}
...
export type CodexScanState = {
  model: string; sessionId: string; project: string; lastUsageSignature: string | null;
  sawSessionMeta: boolean; suppressingForkCopies: boolean; forkCopyAnchorMs: number;
};

export function initialCodexScanState(): CodexScanState {
  return { model: "", sessionId: "", project: "codex", lastUsageSignature: null, sawSessionMeta: false, suppressingForkCopies: false, forkCopyAnchorMs: 0 };
}
...
function isForked(payload: Record<string, unknown>) {
  ...
  return !!spawn && typeof spawn === "object" && typeof (spawn as Record<string, unknown>).parent_thread_id === "string";
}

export function parseCodexUsageLine(line: string, state: CodexScanState): TranscriptUsageRow | undefined {
  let value: unknown;
  try { value = JSON.parse(line); } catch { return undefined; }
  if (!value || typeof value !== "object") return undefined;
  const row = value as Record<string, unknown>;
  ...
  if (row.type === "session_meta") {
    ...
    if (Number.isFinite(timestamp) && isForked(body)) { state.suppressingForkCopies = true; state.forkCopyAnchorMs = timestamp; }
    return undefined;
  }
  ...
  if (!last || typeof last !== "object" || !state.model || typeof row.timestamp !== "string") return undefined;
  ...
  if (state.suppressingForkCopies) {
    if (timestamp - state.forkCopyAnchorMs < 1_000) { state.forkCopyAnchorMs = timestamp; return undefined; }
  ...
  return {
    agent_id: state.sessionId ? `codex-${createHash("sha256").update(state.sessionId).digest("hex").slice(0, 24)}` : `codex:${row.timestamp}`,
    project: state.project, runtime: "codex", model: state.model, tokens,
    uncached_input_tokens: uncached, cached_input_tokens: cached, cache_creation_tokens: cacheWrite,
    output_tokens: output, reasoning_tokens: reasoning, cost_usd: 0, priced: false,
    created_at: row.timestamp, dedupeKey: null,
  };
}

export async function scanClaudeUsage(sinceIso: string, root = join(homedir(), ".claude", "projects")): Promise<{ rows: TranscriptUsageRow[]; files: number; duplicates: number }> {
  ...
  for (const path of files) {
    try {
      ...
      for await (const line of lines) {
        ...
        if (row.dedupeKey && seen.has(row.dedupeKey)) { duplicates += 1; continue; }
        if (row.dedupeKey) seen.add(row.dedupeKey);
        rows.push(row);
      }
    } catch { /* A live transcript may rotate while scanning. */ }
  }
  return { rows, files: files.length, duplicates };
}

export async function scanCodexUsage(sinceIso: string, root = join(homedir(), ".codex", "sessions")): Promise<{ rows: TranscriptUsageRow[]; files: number }> {
  const sinceMs = Date.parse(sinceIso);
  const files = await listJsonl(root, sinceMs);
  const rows: TranscriptUsageRow[] = [];
  for (const path of files) {
    ...
    } catch { /* A live rollout may rotate while scanning. */ }
  }
