...
export const RANGE_DAYS: Record<string, number> = { day: 1, week: 7, month: 30, quarter: 90, year: 365 }
...
export const buildAnalytics = (input: AnalyticsInput) => {
  ...
  for (const row of usage) {
    ...
    const item = projectMap.get(row.project) ??
      { project: row.project, tokens: 0, costUsd: 0, events: 0, sessions: new Set<string>() }
