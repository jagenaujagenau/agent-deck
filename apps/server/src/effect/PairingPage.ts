/**
 * The pairing page itself — the one thing a person sees when they open the
 * bridge to put the deck on a phone.
 *
 * Split from the pairing domain because it is a different kind of thing:
 * `Pairing.ts` decides who may pair and what a pairing link says, and every
 * line of it is under test, while this is a rendering of those answers that
 * is verified by looking at it. Kept together, three hundred lines of markup
 * sat between the rules and the tests that cover them, and the module read
 * as though the page were the point.
 */
