...
export const BridgeRoutes = HttpRouter.addAll([
  // ---- reads -------------------------------------------------------------
  route("GET", "/snapshot", Effect.gen(function* () {
    const state = yield* BridgeState
    return yield* HttpServerResponse.json(yield* state.snapshot)
  })),
  route("GET", "/agents/:agentId/history", Effect.gen(function* () {
    const store = yield* BridgeStore
    return yield* HttpServerResponse.json({ events: yield* store.history(yield* param("agentId")) })
  })),
  route("GET", "/agents/:agentId/changes", Effect.gen(function* () {
    const store = yield* BridgeStore
    return yield* HttpServerResponse.json({ changes: yield* store.fileChanges(yield* param("agentId")) })
  })),
  route("GET", "/agents/:agentId/slash-commands", Effect.gen(function* () {
    const store = yield* BridgeStore
    return yield* HttpServerResponse.json({ commands: yield* store.slashCommands(yield* param("agentId")) })
  })),
  route("GET", "/agents/:agentId/requests/:requestId", Effect.gen(function* () {
    const state = yield* BridgeState
  ...
  route("GET", "/agents/:id/commands", Effect.gen(function* () {
    const state = yield* BridgeState
    const request = yield* HttpServerRequest.HttpServerRequest
  ...
  route("GET", "/commands/:id/receipt", Effect.gen(function* () {
    const state = yield* BridgeState
  ...
  // ---- writes ------------------------------------------------------------
  route("POST", "/agents/heartbeat", Effect.gen(function* () {
    const state = yield* BridgeState
  ...
  route("POST", "/agents/:id/events", Effect.gen(function* () {
    const state = yield* BridgeState
    const input = (yield* body()) as any
    if (!input?.kind || !input?.summary) return yield* error("kind and summary are required", 400)
  ...
  route("POST", "/agents/:id/runtime-events", Effect.gen(function* () {
    const state = yield* BridgeState
    const input = (yield* body()) as any
    const id = yield* param("id")
    if (input?.agentId !== id) return yield* error("Runtime event agent does not match route", 400)
  ...
  route("POST", "/agents/:id/control", Effect.gen(function* () {
    const state = yield* BridgeState
    const input = (yield* body()) as any
    ...
    if ((input.action === "approve" || input.action === "reject") && !(yield* state.hasPendingApproval(id))) {
    ...
    if (command) yield* Effect.forkDetach((yield* ManagedRuntime).handle(command).pipe(Effect.ignore))
    return command === undefined
      ? yield* error("Agent not found", 404)
      : yield* HttpServerResponse.json(command, { status: 202 })
  })),
  route("POST", "/agents/:id/commands/:commandId/ack", Effect.gen(function* () {
    const state = yield* BridgeState
  ...
  route("POST", "/agents/:agentId/slash-commands", Effect.gen(function* () {
    const state = yield* BridgeState
    const input = (yield* body()) as any
   ...
   */
  route("POST", "/agents/:agentId/requests/:requestId/resolve", Effect.gen(function* () {
    const state = yield* BridgeState
    ...
    const resolved = (yield* managed.resolve(agentId, requestId, input.status, input.value)) ||
  ...
  route("GET", "/managed/runtimes", Effect.gen(function* () {
    const managed = yield* ManagedRuntime
    return yield* HttpServerResponse.json({ runtimes: yield* managed.available })
  })),
  route("POST", "/managed/claude/sessions", Effect.gen(function* () {
    const managed = yield* ManagedRuntime
    const input = (yield* body()) as any
  ...
  route("POST", "/managed/:agentId/requests/:requestId/resolve", Effect.gen(function* () {
    const managed = yield* ManagedRuntime
    const config = yield* BridgeConfig
    const input = (yield* body()) as any
    if (!REQUEST_STATUSES.has(input?.status)) return yield* error("Invalid request status", 400)
  ...
  route("GET", "/analytics", Effect.gen(function* () {
    const state = yield* BridgeState
    const request = yield* HttpServerRequest.HttpServerRequest
    const query = new URL(request.url, "http://bridge").searchParams
    return yield* HttpServerResponse.json(
      yield* state.analytics(query.get("range") ?? "month", query.get("project") ?? undefined, query.get("timeZone") ?? "UTC")
    )
  })),
  route("GET", "/diagnostics/projection-parity", Effect.gen(function* () {
    const state = yield* BridgeState
  ...
  route("POST", "/pair", Effect.gen(function* () {
    const state = yield* BridgeState
    const input = (yield* body()) as any
    if (!/^\d{6}$/.test(input?.code) || typeof input?.deviceName !== "string" || !input.deviceName.trim()) {
  ...
  HttpRouter.route("DELETE", `${BRIDGE_PREFIX}/device`, Effect.gen(function* () {
    const state = yield* BridgeState
    const request = yield* HttpServerRequest.HttpServerRequest
    const bearer = (request.headers["authorization"] ?? "").replace(/^Bearer\s+/i, "")
   ...
   */
  route("GET", "/events", Effect.gen(function* () {
    const state = yield* BridgeState
