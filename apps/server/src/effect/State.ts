...
const cardEvent = (event: AgentEvent) => {
  ...
  return { ...rest, detail: rest.detail === undefined ? undefined : clip(rest.detail, SNAPSHOT_DETAIL_LIMIT) }
}
...
export class BridgeState extends Context.Service<BridgeState, {
  ...
  readonly supportsControl: (agentId: string, action: ControlAction) => Effect.Effect<boolean | undefined>
  readonly hasPendingApproval: (agentId: string) => Effect.Effect<boolean>
  readonly pendingCommands: (agentId: string, after?: string) => Effect.Effect<ReadonlyArray<Command>>
  readonly acknowledge: (agentId: string, commandId: string) => Effect.Effect<Command | undefined>
  ...
  readonly commandReceipt: (commandId: string) => Effect.Effect<Record<string, unknown> | undefined>
  readonly analytics: (
    range: string,
    project?: string,
    timeZone?: string
  ...
  static readonly layer = Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const commandRows = yield* sql<{ id: string; data: string }>`SELECT id, data FROM bridge_commands`.pipe(
        Effect.orDie
      )
      ...
      for (const row of commandRows) {
        ...
        } catch { /* Same tolerance as agents. */ }
      }
      ...
      const persistAgent = (agent: AgentRecord) =>
        sql`INSERT INTO bridge_agents (id, data, updated_at)
            ...
            ON CONFLICT(id) DO UPDATE SET data = excluded.data, updated_at = excluded.updated_at`.pipe(Effect.orDie)
      ...
      const persistFileChange = (agentId: string, event: AgentEvent) =>
        event.diff === undefined
          ...
          : sql`INSERT INTO bridge_file_changes (id, agent_id, path, tool, diff, created_at)
                ...
                ON CONFLICT(id) DO UPDATE SET path = excluded.path, tool = excluded.tool, diff = excluded.diff`
              .pipe(Effect.orDie)
      ...
      const setRequestStatus = Effect.fn("BridgeState.setRequestStatus")(function* (
        ...
        const rows = yield* sql<{ data: string }>`SELECT data FROM bridge_requests WHERE request_id = ${requestId}`
        let data: Record<string, unknown> = {}
        try {
          if (rows[0]) data = JSON.parse(rows[0].data) as Record<string, unknown>
        } catch { /* Preserve an empty payload. */ }
      ...
      const pendingApprovalFor = Effect.fn("BridgeState.pendingApprovalFor")(function* (agentId: string) {
        ...
        if (row.expires_at && Date.parse(row.expires_at) <= Date.now()) {
          ...
          yield* recordFact(agentId, "request.resolved", { status: "expired" }, {
      ...
      const snapshot = Effect.gen(function* () {
        ...
        for (const row of projRows) {
          ...
          } catch { /* Compatibility projection remains available. */ }
        }
        ...
        const rendered = [...agents.values()].map((agent) => {
          ...
          return {
            ...
            tokens: activeProjection?.usageKnown
              ...
              : Number.isFinite(agent.tokens) ? agent.tokens : 0,
            processedTokens: activeProjection?.usageKnown
              ? activeProjection.processedTokens
              : Number.isFinite(agent.processedTokens) ? agent.processedTokens : agent.tokens,
        ...
        return {
          ...
          summary: {
            active: rendered.filter((agent) => ["running", "waiting", "paused"].includes(agent.state)).length,
      ...
      const heartbeat = Effect.fn("BridgeState.heartbeat")(function* (input: any) {
        ...
        const cursorRows = yield* sql<{ tokens: number; cost_usd: number }>`
          SELECT tokens, cost_usd FROM bridge_usage_cursors WHERE agent_id = ${agent.id}`.pipe(Effect.orDie)
        ...
        if (tokenDelta > 0 || costDelta > 0) {
          yield* sql`INSERT INTO bridge_usage_deltas (agent_id, project, runtime, tokens, cost_usd, created_at)
                     VALUES (${agent.id}, ${agent.project}, ${runtimeFor(agent)}, ${tokenDelta}, ${costDelta}, ${now()})`
            .pipe(Effect.orDie)
        }
        yield* sql`INSERT INTO bridge_usage_cursors (agent_id, tokens, cost_usd, updated_at)
                   ...
                   ON CONFLICT(agent_id) DO UPDATE SET tokens = MAX(tokens, excluded.tokens),
                     cost_usd = MAX(cost_usd, excluded.cost_usd), updated_at = excluded.updated_at`.pipe(Effect.orDie)
        if (
          agent.runtimeProtocol !== "canonical-v1" &&
          (!previous || previous.state !== agent.state || previous.task !== agent.task ||
        ...
        if (
          agent.runtimeProtocol !== "canonical-v1" && agent.pendingApproval &&
          previous?.pendingApproval?.id !== agent.pendingApproval.id
        ) {
          yield* recordFact(agent.id, "request.opened", {
            kind: "approval",
      ...
      const addEvent = Effect.fn("BridgeState.addEvent")(function* (agentId: string, event: any) {
        ...
        const agent: AgentRecord = { ...previous, events: events.slice(-AGENT_EVENT_WINDOW), lastSeenAt: now() }
        yield* persistAgent(agent)
      ...
      const ingestRuntimeEvent = Effect.fn("BridgeState.ingestRuntimeEvent")(function* (value: unknown) {
        const event = yield* Effect.try({
          try: () => canonicalRuntimeEvent(value),
          catch: (cause) =>
            new InvalidRuntimeEvent({ reason: cause instanceof Error ? cause.message : "Invalid runtime event" })
        })
        ...
        if (event.type === "request.opened" || event.type === "user-input.requested") {
          if (!event.requestId) {
            return yield* new InvalidRuntimeEvent({ reason: "Request lifecycle events require requestId" })
          }
          ...
          yield* upsertRequest(event.agentId, event.requestId, kind, "pending", event.payload, event.createdAt, expiresAt)
        }
        if (event.type === "request.resolved" || event.type === "user-input.resolved") {
          ...
          const status = typeof event.payload.status === "string"
            ...
            : event.type === "user-input.resolved" ? "answered" : "unavailable"
      ...
      const control = Effect.fn("BridgeState.control")(function* (
        ...
        if (existing === undefined || (existing.capabilities && !existing.capabilities.includes(action))) {
          return undefined
        }
        ...
        if ((action === "approve" || action === "reject") && !(yield* hasPendingApproval(agentId))) {
          return undefined
        }
        ...
        if (action === "approve" || action === "reject") {
          ...
          if (request) {
            ...
            yield* recordFact(agentId, "request.resolved", { status }, {
        ...
        yield* persistAgent(agent)
        yield* recordFact(agentId, "session.state.changed", { state: agent.state, commandId: command.id, action })
        yield* changed
        yield* Ref.update(agentsRef, (map) => new Map(map).set(agentId, agent))
        return command
      })
      ...
      const pendingCommands = Effect.fn("BridgeState.pendingCommands")(function* (
        ...
        return [...commands.values()].filter((command) =>
          command.agentId === agentId && !command.acknowledgedAt && Date.parse(command.createdAt) > afterTime
        )
      })

      const acknowledge = Effect.fn("BridgeState.acknowledge")(function* (agentId: string, commandId: string) {
        const commands = yield* Ref.get(commandsRef)
        ...
        yield* persistCommand(command)
        const sequence = yield* recordFact(agentId, "session.state.changed", { commandId, delivery: "acknowledged" })
      ...
      const requestStatus = Effect.fn("BridgeState.requestStatus")(function* (
        ...
        const rows = yield* sql<{ status: RuntimeRequestStatus; data: string; expires_at: string | null }>`
      ...
      const resolveRuntimeRequest = Effect.fn("BridgeState.resolveRuntimeRequest")(function* (
        ...
        yield* recordFact(agentId, "request.resolved", {
          status,
      ...
      const setSlashCommands = Effect.fn("BridgeState.setSlashCommands")(function* (
        agentId: string,
        commands: unknown
      ) {
      ...
      const analytics = Effect.fn("BridgeState.analytics")(function* (
        ...
        const ledgerUsage = yield* sql<UsageRow>`
          ...
          FROM bridge_usage_deltas WHERE created_at >= ${cutoff} ORDER BY created_at`.pipe(Effect.orDie)
        const activityRows = yield* sql<ActivityRow>`
          SELECT agent_id, project, runtime, kind, created_at
          FROM bridge_activity WHERE created_at >= ${cutoff} ORDER BY created_at`.pipe(Effect.orDie)
      ...
      const projectionParity = Effect.gen(function* () {
        const agents = yield* Ref.get(agentsRef)
        const canonical = [...agents.values()].filter((agent) => agent.runtimeProtocol === "canonical-v1")
        const out: Array<Record<string, unknown>> = []
        for (const agent of canonical) {
          const rows = yield* sql<{ sequence: number; data: string }>`
            SELECT sequence, data FROM bridge_runtime_projections WHERE agent_id = ${agent.id}`.pipe(Effect.orDie)
          const row = rows[0]
