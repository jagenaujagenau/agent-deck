...
export class BridgeStore extends Context.Service<BridgeStore, {
  ...
  static readonly layer = Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const agents = Effect.gen(function* () {
        const rows = yield* sql<{ id: string; data: string }>`SELECT id, data FROM bridge_agents`.pipe(
          Effect.orDie
        )
        ...
        for (const row of rows) {
          ...
          decoded.push({ ...agent, events: events.slice(-SNAPSHOT_EVENT_LIMIT).reverse().map(cardEvent) })
        }
        return decoded
      })
