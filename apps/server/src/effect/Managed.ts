...
export class ManagedStartError extends Schema.TaggedError<ManagedStartError>()("ManagedStartError", {
 ...
 */
export class ManagedRuntime extends Context.Service<ManagedRuntime, {
  readonly available: Effect.Effect<ReadonlyArray<Record<string, unknown>>>
  readonly start: (input: StartInput) => Effect.Effect<Record<string, unknown>, ManagedStartError>
  readonly resolve: (
    agentId: string,
    requestId: string,
    ...
    Effect.gen(function* () {
        ...
        resolve: async (requestId, status, value) => {
          await run(Effect.gen(function* () {
            const rows = yield* sql<{ agent_id: string }>`
              SELECT agent_id FROM bridge_requests WHERE request_id = ${requestId}`.pipe(Effect.orDie)
          ...
          while (!signal.aborted) {
            const row = await run(Effect.gen(function* () {
              const rows = yield* sql<{ status: RuntimeRequestStatus; data: string; expires_at: string | null }>`
                SELECT status, data, expires_at FROM bridge_requests WHERE request_id = ${requestId}`.pipe(Effect.orDie)
              return rows[0]
            }))
            ...
            if (row.expires_at && Date.parse(row.expires_at) <= Date.now()) {
              await run(Effect.gen(function* () {
                yield* sql`UPDATE bridge_requests SET status = 'expired', resolved_at = ${now()}
                           WHERE request_id = ${requestId} AND status = 'pending'`.pipe(Effect.orDie)
              }))
      ...
      const consumeEvent = (agentId: string, hosted: Hosted, event: any) =>
        Effect.gen(function* () {
          ...
          if (event.type === "token-usage.updated") {
            ...
            const turnTokens = num("input_tokens") + num("cache_read_input_tokens") +
              num("cache_creation_input_tokens") + num("output_tokens")
          ...
          if (event.type === "user-input.requested") {
            ...
            const options = questions.length === 1 && first?.multiSelect !== true && Array.isArray(first?.options)
          ...
          if (event.type === "item.started" || event.type === "item.completed" || event.type === "runtime.error") {
            const kind = event.type === "runtime.error"
              ? "error"
            ...
            const detail = (kind === "output" || kind === "thought") && typeof event.payload.text === "string"
              ? event.payload.text
              : toolInput
            ...
            yield* state.addEvent(agentId, {
              ...
              path: typeof toolInput?.path === "string"
                ? toolInput.path
      ...
      const start = Effect.fn("ManagedRuntime.start")(function* (input: StartInput) {
        if (!input.project?.trim() || !input.cwd?.startsWith("/")) {
          return yield* new ManagedStartError({ message: "project and an absolute cwd are required" })
        }
        const directory = yield* Effect.promise(() => stat(input.cwd).catch(() => undefined))
        if (!directory?.isDirectory()) {
          return yield* new ManagedStartError({ message: "cwd does not exist or is not a directory" })
        }
        ...
        const unattended = ["auto", "bypassPermissions", "dontAsk"].includes(input.permissionMode ?? "default")
        ...
        if (input.prompt?.trim()) {
          yield* Effect.tryPromise({
            try: () => adapter.send(session, input.prompt!.trim()),
              ...
              new ManagedStartError({
                message: cause instanceof Error ? cause.message : "Unable to send the initial prompt"
              })
          })
        }
        return {
      ...
      const resolve = Effect.fn("ManagedRuntime.resolve")(function* (
        ...
        yield* Effect.promise(() => adapter.resolveRequest(hosted.session, requestId, status, value))
        return true
      })
