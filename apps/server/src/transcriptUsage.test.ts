...
describe("Codex transcript usage", () => {
  test("uses last usage deltas without double-counting cached input", () => {
    ...
    parseCodexUsageLine(
      JSON.stringify({
        ...
        payload: { id: "cx1", cwd: "/work/deck" },
      }),
      state,
    );
    parseCodexUsageLine(
      JSON.stringify({ type: "turn_context", payload: { model: "gpt-5", cwd: "/work/deck" } }),
      state,
    );
  ...
  test("suppresses copied history at the start of a fork", () => {
    ...
    const usage = (timestamp: string, output: number) =>
      JSON.stringify({
        timestamp,
        payload: { type: "token_count", info: { last_token_usage: { output_tokens: output } } },
      });
