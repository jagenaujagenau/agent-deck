...
const program = Effect.gen(function* () {
  ...
  for (const id of ids) {
    for (const [label, live, mine] of [
      ["history", (yield* Effect.promise(() => get(`/agents/${id}/history`))).events,
                  yield* store.history(id)],
      ["changes", (yield* Effect.promise(() => get(`/agents/${id}/changes`))).changes,
                  yield* store.fileChanges(id)],
      ["commands", (yield* Effect.promise(() => get(`/agents/${id}/slash-commands`))).commands,
                   yield* store.slashCommands(id)]
    ] as Array<[string, any, any]>) {
      const a = JSON.stringify(live), b = JSON.stringify(mine)
      if (a === b) { pass++ }
      else {
        fail++
        console.log(`MISMATCH ${label} ${id.slice(0,8)}: live=${live?.length} mine=${mine?.length}`)
        if (live?.length === mine?.length && live?.length) {
          const i = live.findIndex((_: any, k: number) => JSON.stringify(live[k]) !== JSON.stringify(mine[k]))
...
Effect.runPromise(program.pipe(Effect.provide(BridgeStore.layer.pipe(Layer.provide(SqlLive)))))
