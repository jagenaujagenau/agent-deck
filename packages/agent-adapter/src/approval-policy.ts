...
export function normalizeApprovalMode(value: string | undefined): ApprovalMode {
  return value && ["off", "destructive", "all"].includes(value) ? value as ApprovalMode : "destructive";
}
...
const SENSITIVE_PATH_PATTERN = /(?:^|\/)(?:\.env(?:\.[^/]+)?|credentials?|secrets?|auth\.json|.*\.pem|.*\.key)$/i;

export function requiresApproval(toolName: string, input: Record<string, unknown>, mode: ApprovalMode): boolean {
