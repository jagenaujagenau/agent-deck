/**
 * The full lifecycle-event handler, shared by both paths that can serve one.
 *
 * The per-session daemon calls this from its socket server — the fast path,
 * where the module graph is already loaded. The hook shim dynamic-imports it
 * only when no daemon answered, which is what keeps this file's imports off
 * the per-event startup cost. Either way, one call is one hook invocation:
 * state is loaded from the session file at the top and saved as the event
 * demands, exactly as the per-process script this replaces did.
 */
...
export type HookEventRequest = {
  ...
  /** The hook's stdin, verbatim. */
  payloadText: string;
  /** The hook process's cwd — the fallback when the payload names none. */
  hookCwd: string;
  /**
   * The hook process's parent pid. The daemon cannot use its own — it was
   * reparented when the hook that spawned it exited — and the owner-pid walk
   * has to start from the process the runtime actually forked.
   */
  hookPpid: number;
};

export type HookEventResult = { stdout?: string };
...
export async function handleHookEvent(request: HookEventRequest): Promise<HookEventResult> {
  const out: string[] = [];
  try {
    await handle(request, (line) => out.push(line));
  } catch (cause) {
    // A decision already emitted outranks the failure: a Stop delivery's
    // message is acknowledged before its stdout, so the runtime must still
    // see the decision even when a later publish threw.
    if (out.length === 0) throw cause;
  }
...
async function handle(request: HookEventRequest, emit: (line: string) => void): Promise<void> {
  ...
  try {
    // SAFETY: the state file is this adapter's own — every writer is this
    // handler or the daemon's heartbeat loop, and both serialise exactly this
    // shape. A corrupt file fails the parse and falls through to the fresh
    // state below. Loaded fresh per event, never cached across them: the
    // daemon serving a stream of events must see each one's predecessor's
    // writes, the same way one-process-per-hook did.
  ...
  function updateUsageFromTranscript() {
    ...
    // Gemini's conversation file is one JSON document rewritten in place, not
    // a JSONL of usage entries — it gets its own reader.
    if (runtime === "gemini") {
      const read = readGeminiConversation(input.transcriptPath, sessionKey);
      state.model = read.model ?? state.model;
      if (read.processedTokens > 0) {
        state.tokens = read.contextTokens;
        state.processedTokens = read.processedTokens;
      }
      return;
    }
    try {
  ...
  const publishRuntime = (
    ...
    // The daemon's heartbeat loop publishes state reports for this session
    // too, so every report carries the shared counter — one total order per
    // session, whichever code path spoke. Persisted before the wire: a report
  ...
  switch (event) {
    ...
    case "PreToolUse": {
      ...
      save();
      ensureDaemon();
      // preToolUse saves on every branch; this was the per-process exit point.
      await preToolUse();
      return;
    }
    case "PostToolUse": {
    ...
    case "Notification": {
      ...
      // flow is genuinely holding, whatever this notification happens to say.
    ...
    case "Stop":
      ...
      // Gemini's reasoning lives only in its conversation file, which is
      // rewritten in place and so cannot be tailed; one read per turn here is
      // the honest cadence. Ids derive from message ids, so re-fires collapse.
      if (runtime === "gemini" && input.transcriptPath) {
        for (const thought of readGeminiConversation(input.transcriptPath, sessionKey).thoughts) {
          await publish("thought", "Reasoning", thought.text, { id: thought.id }).catch(() => {});
        }
      }
      state.activeTurnId = undefined;
      {
        ...
        if (messages.length > 0) {
          ...
          state.activeTurnId = crypto.randomUUID();
          save();
          // Emitted before the network: the message is already acknowledged, so a bridge hiccup
          // after this line must not be what loses it — an emitted decision survives a later throw.
          emit(stopHookDecision(messages));
          await publishRuntime(
            "turn.started",
            { objective: state.objective },
            { turnId: state.activeTurnId },
